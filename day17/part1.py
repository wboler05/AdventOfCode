                ...
                for m in range(max(i-1, 0), min(i+2, c_data.shape[0])):
                    for n in range(max(j-1, 0), min(j+2, c_data.shape[1])):
                        for p in range(max(k-1, 0), min(k+2, c_data.shape[2])):
                            visits += 1
                            if (m == i) and (n == j) and (p == k):
                                continue
                            if c_data[m,n,p] == ACTIVE_STATE:
